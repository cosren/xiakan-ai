# 🧠 Narrator AI CLI Skill + Xiakan 内容工厂

[English](README.md)

> 本仓库整合了 `narrator-ai-cli` 的 AI Agent 技能与 `xiakan-content-factory` 的命令插件。你既可以让 AI 生成电影解说视频，也可以快速生成《瞎看什么》风格的脚本与结构化时间轴。

## 这是什么？

包含两部分能力：

- **`narrator-ai-cli-skill`**：一套 AI Agent 技能说明 (`SKILL.md`)，教 AI 如何调用 `narrator-ai-cli` 完成电影解说视频制作。
- **`xiakan-content-factory`**：`src/` 目录下的 CLI 实现，用于生成高密度剧情脚本和结构化卡点 JSON 时间轴。
